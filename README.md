...
GPU-accelerated partitioned overlap-save (UPOLS) convolution applied at the full DSD playback rate. Designed for room correction filters generated by external tools (REW, Acourate, Audiolense, Dirac Live).

### How it works

Convolution runs as a stage in the DSP pipeline AFTER FIR rate conversion (or boxcar smoothing for same-rate paths) and BEFORE SDM re-encoding:

```
1-bit DSD in → unpack → boxcar/FIR → CONV → gain → SDM → 1-bit DSD out
                                      ↑
                                 IR loaded from
                                 user WAV file
```

The IR is loaded from a WAV file (mono or first channel of stereo), resampled to the playback DSD rate, normalized to unity passband peak gain, and partitioned in the frequency domain for efficient block convolution.

Pipeline (`conv_load_ir` in `convolution.c`):

1. **Load** WAV file (any sample rate, IEEE float or PCM int)
2. **Pre-truncate input** if the post-resample size would exceed the budget cap — picks a centered window around the impulse peak in the input samples. Avoids OOM on long input IRs at high DSD rates.
3. **Resample** to the playback DSD rate using IPP FIRMR (power-of-2 ratios) or libsoxr/IPP polyphase (other ratios)
4. **Normalize** to unity passband peak gain (FFT-based peak detection — works for any filter type)
5. **Safety clamp**: peak ≤ 10.0 to prevent speaker damage from corrupt filters
6. **Centered post-resample truncation** if still > cap (linear-phase safe)
7. **Partition** into N partitions of size P, FFT each partition (UPOLS frequency-domain IR storage)
8. **Convolve per batch**: each 200ms audio batch is partitioned and convolved against the frequency-domain IR via overlap-save block convolution

The convolution operates **at the full DSD playback rate** (e.g., 22.6 MHz at DSD512). No decimation, no rate-conversion quality loss.

### CPU implementation (`convolution.c`)

Used when CUDA is unavailable or `Convolution → CUDA GPU` is unchecked.

- **Partition size**: 1024–16384 taps (auto-selected from IR length)
- **FFT engine**: Intel IPP `ippsDFT_C_64fc` (fp64 complex)
- **Threading**: single-threaded — the SDM Par4 path already saturates the CPU at high DSD rates
- **FDL**: frequency-domain delay line of `num_partitions` slots; new input chunks rotate through circularly
- **Per chunk**: pack input → forward FFT → multiply × accumulate against IR partitions → inverse FFT → output the second half (overlap-save valid region)

CPU conv works at all rates but limits practical IR length at high rates because each chunk dispatches serially.

### GPU implementation (`gpu_cuda.c`, `kernels/conv_kernels.cu`)

The fast path. Used when CUDA is available, `gpu_enabled` is set, and `Convolution → CUDA GPU` is checked.

- **Partition size**: 65536 taps (single-IR-partition mode for IRs ≤ 64K — minimizes per-chunk driver dispatch)
- **FFT engine**: cuFFT `cufftExecZ2Z` (complex double precision), one plan per channel bound to the channel's CUDA stream
- **Per-channel CUDA streams**: ch0 → stream 0, ch1 → stream 1. Both channels' GPU work overlaps on the device.
- **Pipelined launch + finalize**: `conv_launch` queues all kernel work asynchronously and returns immediately. `conv_finalize` syncs the stream and drains the output. The CPU SDM phase runs IN PARALLEL with the GPU conv work.
- **Single batch upload**: ONE pinned `cuMemcpyHtoDAsync` per batch from a sliding-window pinned host buffer (vs N per-chunk uploads)
- **Single batch download**: extract kernel writes directly into a consolidated `d_output_flat` device buffer; ONE `cuMemcpyDtoHAsync` per batch (vs N per chunk)
- **Inline FDL rotation**: the fused multiply-accumulate kernel computes its FDL slot inline from `chunk_fdl_pos` (passed as a kernel argument) — no per-chunk index upload
- **CUDA Graph capture**: when `batch_count % num_partitions == 0` (the common case for DSD512 64K-256K, DSD256 1M, etc.), the entire per-batch dispatch (upload + N chunks + download) is captured as a CUDA graph and replayed via a single `cuGraphLaunch`. CPU dispatch cost drops from ~5ms/channel (loop) to ~0.3ms/channel (graph replay)
- **4-slot graph cache** keyed by (`batch_count`, `init_fdl_pos`) — handles the natural `batch_count` oscillation (e.g., 68/69 at DSD512) caused by leftover sample carry-over

The GPU implementation is ~10× faster than the CPU path on a typical RTX-class card and is the recommended path. With CUDA Graph capture and all dispatch optimizations enabled, total per-batch conv overhead drops from 30+ ms (per channel) to ~0.5 ms (per channel) on an RTX 5080.

### Time budget per rate (HIGH budget)

The cap (`max_ir_taps`) is a **time budget**, not an arbitrary limit. It's the largest post-resample IR length that fits in the 200ms batch budget on an RTX 5080. Different rate × SDM mode combinations have different time budgets because per-chunk fused mul-acc work and SDM cost scale differently.

| Rate | Trellis cap | Trellis time | PreCorr cap | PreCorr time |
|---|---|---|---|---|
| DSD64  | 4M | **1.42 s** | 4M | **1.42 s** |
| DSD128 | 4M | **708 ms** | 4M | **708 ms** |
| DSD256 | 1M | **88 ms**  | 4M | **354 ms** |
| DSD512 | 2M | **88 ms**  | 2M | **88 ms** |

The cap is applied **post-resample** — after the IR is resampled to the DSD playback rate. A 32K-tap IR at 44.1 kHz becomes 16M post-resample at DSD512 (way over the 2M cap), so it's pre-truncated to a centered 4160-tap input window (= 94 ms at 44.1 kHz) before resampling.

**Why DSD256/512 have shorter time budgets**: these rates have far more conv chunks per batch (17 / 68) than DSD64/128 (4 / 17), so per-chunk fused-mul-acc cost scales fastest with `num_partitions`. DSD256+Trellis additionally has expensive Trellis lat=128 SDM which leaves less headroom.

**To preserve more of your IR at high DSD rates**: use **PreCorr** instead of Trellis (cheaper SDM = more headroom for conv) or play at a lower DSD rate (lower rates have proportionally larger time budgets).

### Budget settings

The DSP settings dialog has a **Budget: High / Medium / Low** selector under Convolution that controls the cap:

- **High** — full cap as listed above (max IR length, recommended)
- **Medium** — cap / 2 (half the IR length)
- **Low** — cap / 4 (quarter)
...
|---|---|---|
| DSD64  | 22-134 MB (depends on actual IR length) | 22-134 MB |
| DSD128 | 22-134 MB | 22-134 MB |
| DSD256 | 70 MB | 134 MB |
...
[Room EQ Wizard](https://www.roomeqwizard.com/) is the standard free tool for room correction. The plugin accepts any WAV file containing an impulse response.

1. **Measure your room**: REW → Measure. Use a USB measurement mic (UMIK-1, UMIK-2). Take a few sweeps at the listening position.
2. **Create the EQ**: REW → EQ tab. Set:
   - **Equaliser**: Generic
   - **Target**: Match a target curve (Harman, B&K, custom)
   - **Filter Tasks → Match Response to Target**: generates the biquad cascade
3. **Generate the IR**:
   - REW → File → Export → **Filter impulse response as WAV**
   - **Sample rate**: match your typical DSD playback family — **44100 Hz** for DSD/44, **48000 Hz** for DSD/48. The plugin upsamples cleanly at power-of-2 ratios.
   - **Length**: 8K-32K taps is plenty for most rooms. Longer IRs add late reverb compensation that's likely to be truncated by the cap at high DSD rates anyway.
   - **Format**: 32-bit float (no quantization noise)
   - **Channels**: mono — export L and R **separately**
4. **Load in foobar2000**: DSP Settings → DSD Trellis → Convolution → Enable → click ⋯ next to L and R to browse for the WAV files. Click OK.
