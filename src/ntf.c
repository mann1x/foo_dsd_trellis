/*
 * foo_dsd_trellis — NTF coefficient tables
 * Ported from mansr/sox sdm.c (LGPL v2.1+)
 *
 * 40 filters: 4 frequency groups (DSD64/128/256/512) × 5 orders (4–8) × 2 variants (CLANS/SDM)
 * Ordered: 512×44100 first, then 256×44100, then 128×44100, then 64×44100.
 * Within each group: order ascending, CLANS before SDM at each order.
 *
 * DSD512 filters generated by tools/gen_ntf_512.py:
 *   g[] scaled ÷4 from 256×44100; a[] derived analytically preserving NTF poles.
 */

#include "../include/ntf.h"
#include <string.h>

static const ntf_filter_t sdm_filters[] = {
  /* ═══════════════════════════════════════════════════════════════
   * 2048 × 44100  (DSD2048 = 90316800 Hz)
   * Generated by tools/gen_ntf_pydelsig.py (synthesizeNTF opt=1 H_inf=1.5)
   * ═══════════════════════════════════════════════════════════════ */
  { { 5.55556151114022123e-01, 2.49969151124973782e-01, 5.24265279653165356e-02, 6.14174321898160267e-03 },
    { 1.74495239302352445e-06, 0, 2.71987682376462203e-07, 0 },
    4, 2048 * 44100, "clans-4" },
  { { 5.55556136990804528e-01, 2.52128254639531424e-01, 5.55458862865666828e-02, 8.86730786856791339e-03, 6.55486768177230856e-04 },
    { 1.93227337064172389e-06, 0, 6.82278565644622859e-07, 0, 0 },
    5, 2048 * 44100, "sdm-5" },
  { { 5.55556268794057950e-01, 2.53192213573345426e-01, 5.70831268039849252e-02, 1.02619603755165541e-02, 1.05131824439548484e-03, 5.67580204923452892e-05 },
    { 2.04601649533842078e-06, 0, 1.02877042840177069e-06, 0, 1.33984841443890446e-07, 0 },
    6, 2048 * 44100, "clans-6" },
  { { 5.55556268794057950e-01, 2.53192213573345426e-01, 5.70831268039849252e-02, 1.02619603755165541e-02, 1.05131824439548484e-03, 5.67580204923452892e-05 },
    { 2.04601649533842078e-06, 0, 1.02877042840177069e-06, 0, 1.33984841443890446e-07, 0 },
    6, 2048 * 44100, "sdm-6" },
  { { 5.55556467613517291e-01, 2.53800951157403865e-01, 5.79627101037097497e-02, 1.10720636083129088e-02, 1.29481365572432891e-03, 1.05620175418119790e-04, 4.13925782422831754e-06 },
    { 2.11968289343111849e-06, 0, 1.29389239256205713e-06, 0, 3.87576525984201226e-07, 0, 0 },
    7, 2048 * 44100, "sdm-7" },
  { { 5.55556169750026974e-01, 2.54182978130485238e-01, 5.85154866803511420e-02, 1.15853903065059154e-02, 1.45357547789070349e-03, 1.42106717266820805e-04, 8.48102727766314246e-06, 2.60840926359763238e-07 },
    { 2.16992394186377169e-06, 0, 1.49345887545671019e-06, 0, 6.49891033432936638e-07, 0, 7.91797529764437513e-08, 0 },
    8, 2048 * 44100, "sdm-8" },

  /* ═══════════════════════════════════════════════════════════════
   * 1024 × 44100  (DSD1024 = 45158400 Hz)
   * Generated by tools/gen_ntf_pydelsig.py (synthesizeNTF opt=1 H_inf=1.5)
   * ═══════════════════════════════════════════════════════════════ */
  { { 5.55557495741890395e-01, 2.49964720514104066e-01, 5.24265847927817552e-02, 6.14162130712694963e-03 },
    { 6.97980652675234126e-06, 0, 1.08795065512090616e-06, 0 },
    4, 1024 * 44100, "clans-4" },
  { { 5.55557880020563943e-01, 2.52122542687773010e-01, 5.55454520905887816e-02, 8.86694783111492343e-03, 6.55501301393289189e-04 },
    { 7.72908974866481913e-06, 0, 2.72911379739504412e-06, 0, 0 },
    5, 1024 * 44100, "sdm-5" },
  { { 5.55558407967560619e-01, 2.53185222419187173e-01, 5.70820766994123152e-02, 1.02613001999697293e-02, 1.05132399530722623e-03, 5.67557775033865645e-05 },
    { 8.18406179559083569e-06, 0, 4.11508065556454028e-06, 0, 5.35939347345859574e-07, 0 },
    6, 1024 * 44100, "clans-6" },
  { { 5.55558407967560619e-01, 2.53185222419187173e-01, 5.70820766994123152e-02, 1.02613001999697293e-02, 1.05132399530722623e-03, 5.67557775033865645e-05 },
    { 8.18406179559083569e-06, 0, 4.11508065556454028e-06, 0, 5.35939347345859574e-07, 0 },
    6, 1024 * 44100, "sdm-6" },
  { { 5.55559002540473745e-01, 2.53792683239629946e-01, 5.79609767522394001e-02, 1.10710694264764941e-02, 1.29478819887054140e-03, 1.05611473595293356e-04, 4.13944961717887431e-06 },
    { 8.47872708065189329e-06, 0, 5.17556789625395197e-06, 0, 1.55030595339056276e-06, 0, 0 },
    7, 1024 * 44100, "sdm-7" },
  { { 5.55559097071998353e-01, 2.54173431008977324e-01, 5.85130294096365133e-02, 1.15840418956018286e-02, 1.45350190234021345e-03, 1.42087878442133755e-04, 8.48113632554970103e-06, 2.54758348223128730e-07 },
    { 8.67969105899923932e-06, 0, 5.97383327116673968e-06, 0, 2.59956371184699719e-06, 0, 3.16719005466481462e-07, 0 },
    8, 1024 * 44100, "sdm-8" },

  /* ═══════════════════════════════════════════════════════════════
   * 512 × 44100  (DSD512 = 22579200 Hz)
   * Generated by tools/gen_ntf_512.py — resonator g[] scaled ÷4
   * from 256×44100; a[] derived analytically preserving NTF poles.
   * ═══════════════════════════════════════════════════════════════ */

  /* [0] clans-4 @ 512×44100 */
  {
    {
      1.00304549621278261e+00,
      3.55038000265185449e-01,
      5.65539566605016628e-02,
      4.02001307146105519e-03,
    },
    {
      4.35177776403212522e-06, 0,
      2.79182030498607516e-05, 0,
    },
    4, 512 * 44100, "clans-4"
  },

  /* [1] sdm-4 @ 512×44100 */
  {
    {
      8.69565407754900876e-01,
      3.58142683649154026e-01,
      8.03304776962072975e-02,
      8.09449356178279164e-03,
    },
    {
      4.35177776403212522e-06, 0,
      2.79182030498607516e-05, 0,
    },
    4, 512 * 44100, "sdm-4"
  },

  /* [2] clans-5 @ 512×44100 */
  {
    {
      1.10185696470130812e+00,
      4.33518225030791626e-01,
      7.19157961090027831e-02,
      4.52331197854410200e-03,
      9.27336923732685354e-05,
    },
    {
      0, 1.09162987807501495e-05,
      0, 3.09151044987402524e-05,
    },
    5, 512 * 44100, "clans-5"
  },

  /* [3] sdm-5 @ 512×44100 */
  {
    {
      8.07541540076498809e-01,
      3.16525867885544321e-01,
      7.39152683431405916e-02,
      1.01712775801687163e-02,
      6.53424360050231055e-04,
    },
    {
      0, 1.09162987807501495e-05,
      0, 3.09151044987402524e-05,
    },
    5, 512 * 44100, "sdm-5"
  },

  /* [4] clans-6 @ 512×44100 */
  {
    {
      9.96692595627610034e-01,
      3.46103578879556228e-01,
      5.75734801701395724e-02,
      5.01227374758593785e-03,
      2.21928758933108504e-04,
      3.94718571150696492e-06,
    },
    {
      2.14375135770961981e-06, 0,
      1.64599670133086745e-05, 0,
      3.27348406489482479e-05, 0,
    },
    6, 512 * 44100, "clans-6"
  },

  /* [5] sdm-6 @ 512×44100 */
  {
    {
      8.08573400545090304e-01,
      3.20519418684104274e-01,
      7.86931113787880804e-02,
      1.25234934402169672e-02,
      1.21956075694055013e-03,
      5.63800547409295616e-05,
    },
    {
      2.14375135770961981e-06, 0,
      1.64599670133086745e-05, 0,
      3.27348406489482479e-05, 0,
    },
    6, 512 * 44100, "sdm-6"
  },

  /* [6] clans-7 @ 512×44100 */
  {
    {
      1.10591508317190734e+00,
      4.22241107784868341e-01,
      7.56476245655696572e-02,
      7.13951613409408853e-03,
      3.65395855646322154e-04,
      9.61644475239682951e-06,
      9.42492443129939181e-08,
    },
    {
      0, 6.20117334174287477e-06,
      0, 2.07017090743089502e-05,
      0, 3.39133986833962522e-05,
    },
    7, 512 * 44100, "clans-7"
  },

  /* [7] sdm-7 @ 512×44100 */
  {
    {
      7.82459862164608722e-01,
      3.02016020837914068e-01,
      7.37887883074946210e-02,
      1.22540599902002112e-02,
      1.37754878054292254e-03,
      9.70063359024610645e-05,
      3.27111259815228135e-06,
    },
    {
      0, 6.20117334174287477e-06,
      0, 2.07017090743089502e-05,
      0, 3.39133986833962522e-05,
    },
    7, 512 * 44100, "sdm-7"
  },

  /* [8] clans-8 @ 512×44100 */
  {
    {
      1.15143264272600732e+00,
      5.45150096055313838e-01,
      1.38912818941889699e-01,
      2.08161063681192790e-02,
      1.87906984686891064e-03,
      9.99655232645629415e-05,
      2.88522485788018093e-06,
      3.24179527326918679e-08,
    },
    {
      1.26687391565648508e-06, 0,
      1.03981129354227995e-05, 0,
      2.38945836736217758e-05, 0,
      3.47171821856602481e-05, 0,
    },
    8, 512 * 44100, "clans-8"
  },

  /* [9] sdm-8 @ 512×44100 */
  {
    {
      7.41962345503438092e-01,
      2.72662546091641977e-01,
      6.42825266369799336e-02,
      1.05841354983900356e-02,
      1.24280571761475694e-03,
      1.01331249045743415e-04,
      5.25928525531518707e-06,
      1.30868513160665645e-07,
    },
    {
      1.26687391565648508e-06, 0,
      1.03981129354227995e-05, 0,
      2.38945836736217758e-05, 0,
      3.47171821856602481e-05, 0,
    },
    8, 512 * 44100, "sdm-8"
  },

  /* ═══════════════════════════════════════════════════════════════
   * 256 × 44100  (DSD256 = 11289600 Hz)
   * ═══════════════════════════════════════════════════════════════ */

  /* [0] clans-4 @ 256×44100 */
  {
    {
      1.00323940832478e+00,
      3.54975562370606e-01,
      5.64754047673194e-02,
      3.99067228430322e-03,
    },
    {
      1.74071110561285e-05, 0,
      1.11672812199443e-04, 0,
    },
    4, 256 * 44100, "clans-4"
  },

  /* [1] sdm-4 @ 256×44100 */
  {
    {
      8.69746397840960e-01,
      3.58080546314756e-01,
      8.02654082306273e-02,
      8.06528716282692e-03,
    },
    {
      1.74071110561285e-05, 0,
      1.11672812199443e-04, 0,
    },
    4, 256 * 44100, "sdm-4"
  },

  /* [2] clans-5 @ 256×44100 */
  {
    {
      1.10212073518628e+00,
      4.33447134954244e-01,
      7.17865111532609e-02,
      4.48367825425951e-03,
      8.60861641068938e-05,
    },
    {
      0, 4.36651951230006e-05,
      0, 1.23660417994961e-04,
    },
    5, 256 * 44100, "clans-5"
  },

  /* [3] sdm-5 @ 256×44100 */
  {
    {
      8.07768375734983e-01,
      3.16440095967511e-01,
      7.38231738259889e-02,
      1.01432044963374e-02,
      6.46658652275506e-04,
    },
    {
      0, 4.36651951230006e-05,
      0, 1.23660417994961e-04,
    },
    5, 256 * 44100, "sdm-5"
  },

  /* [4] clans-6 @ 256×44100 */
  {
    {
      9.97000121097967e-01,
      3.46002867430604e-01,
      5.74352078895161e-02,
      4.96197900435677e-03,
      2.16319301330580e-04,
      3.45938007947910e-06,
    },
    {
      8.57500543083848e-06, 0,
      6.58398680532347e-05, 0,
      1.30939362595793e-04, 0,
    },
    6, 256 * 44100, "clans-6"
  },

  /* [5] sdm-6 @ 256×44100 */
  {
    {
      8.08851952379691e-01,
      3.20414766828429e-01,
      7.85858596284593e-02,
      1.24781319607895e-02,
      1.21202847406105e-03,
      5.51622876557856e-05,
    },
    {
      8.57500543083848e-06, 0,
      6.58398680532347e-05, 0,
      1.30939362595793e-04, 0,
    },
    6, 256 * 44100, "sdm-6"
  },

  /* [6] clans-7 @ 256×44100 */
  {
    {
      1.10629931445134e+00,
      4.22135693734657e-01,
      7.54595882135669e-02,
      7.07164815703843e-03,
      3.53092575577382e-04,
      8.89662856104825e-06,
      5.79674109824069e-08,
    },
    {
      0, 2.48046933669715e-05,
      0, 8.28068362972358e-05,
      0, 1.35653594733585e-04,
    },
    7, 256 * 44100, "clans-7"
  },

  /* [7] sdm-7 @ 256×44100 */
  {
    {
      7.82785077952658e-01,
      3.01888671316811e-01,
      7.36594376027782e-02,
      1.22068270909817e-02,
      1.36572694403914e-03,
      9.57806082134936e-05,
      3.13239368043838e-06,
    },
    {
      0, 2.48046933669715e-05,
      0, 8.28068362972358e-05,
      0, 1.35653594733585e-04,
    },
    7, 256 * 44100, "sdm-7"
  },

  /* [8] clans-8 @ 256×44100 */
  {
    {
      1.15188624720851e+00,
      5.45054196257555e-01,
      1.38703640845632e-01,
      2.07076444822072e-02,
      1.85506614417771e-03,
      9.63403135615390e-05,
      2.69174565706992e-06,
      2.22594461751768e-08,
    },
    {
      5.06749566262594e-06, 0,
      4.15924517416912e-05, 0,
      9.55783346944871e-05, 0,
      1.38868728742641e-04, 0,
    },
    8, 256 * 44100, "clans-8"
  },

  /* [9] sdm-8 @ 256×44100 */
  {
    {
      7.42329617949054e-01,
      2.72509195471757e-01,
      6.41424039739473e-02,
      1.05299412132258e-02,
      1.23178223428228e-03,
      9.94985029720342e-05,
      5.13169547054423e-06,
      1.20466411041020e-07,
    },
    {
      5.06749566262594e-06, 0,
      4.15924517416912e-05, 0,
      9.55783346944871e-05, 0,
      1.38868728742641e-04, 0,
    },
    8, 256 * 44100, "sdm-8"
  },

  /* ═══════════════════════════════════════════════════════════════
   * 128 × 44100  (DSD128 = 5644800 Hz)
   * ═══════════════════════════════════════════════════════════════ */

  /* [10] clans-4 @ 128×44100 */
  {
    {
      1.19985242167687e+00,
      5.39366678861047e-01,
      1.07433710905069e-01,
      7.85649993434925e-03,
    },
    {
      6.96272321944526e-05, 0,
      4.46641365529834e-04, 0,
    },
    4, 128 * 44100, "clans-4"
  },

  /* [11] sdm-4 @ 128×44100 */
  {
    {
      8.69935494013007e-01,
      3.57844753369190e-01,
      8.00232187246903e-02,
      7.95176796646842e-03,
    },
    {
      6.96272321944526e-05, 0,
      4.46641365529834e-04, 0,
    },
    4, 128 * 44100, "sdm-4"
  },

  /* [12] clans-5 @ 128×44100 */
  {
    {
      1.12849522129362e+00,
      5.02128177800632e-01,
      1.10084368682902e-01,
      1.18635667860902e-02,
      4.71059243536326e-04,
    },
    {
      0, 1.74653153894942e-04,
      0, 4.94580504383930e-04,
    },
    5, 128 * 44100, "clans-5"
  },

  /* [13] sdm-5 @ 128×44100 */
  {
    {
      8.08016362125685e-01,
      3.16129639744972e-01,
      7.34835047943110e-02,
      1.00377576971692e-02,
      6.20309683440734e-04,
    },
    {
      0, 1.74653153894942e-04,
      0, 4.94580504383930e-04,
    },
    5, 128 * 44100, "sdm-5"
  },

  /* [14] clans-6 @ 128×44100 */
  {
    {
      1.13839804508630e+00,
      5.16338264778321e-01,
      1.20760874713903e-01,
      1.53496744585395e-02,
      1.00733946588732e-03,
      2.18223963130981e-05,
    },
    {
      3.42997276004814e-05, 0,
      2.63342132660038e-04, 0,
      5.23688869916463e-04, 0,
    },
    6, 128 * 44100, "clans-6"
  },

  /* [15] sdm-6 @ 128×44100 */
  {
    {
      8.09157514480151e-01,
      3.20038611545599e-01,
      7.81955723892726e-02,
      1.23074728674017e-02,
      1.18346416730106e-03,
      5.04301224894810e-05,
    },
    {
      3.42997276004814e-05, 0,
      2.63342132660038e-04, 0,
      5.23688869916463e-04, 0,
    },
    6, 128 * 44100, "sdm-6"
  },

  /* [16] clans-7 @ 128×44100 */
  {
    {
      8.98180853333862e-01,
      3.27985497323439e-01,
      6.38803466871112e-02,
      7.18262647412857e-03,
      4.51845004995476e-04,
      1.49685651672331e-05,
      4.22554681245302e-08,
    },
    {
      0, 9.92163123766340e-05,
      0, 3.31199917300393e-04,
      0, 5.42540771343282e-04,
    },
    7, 128 * 44100, "clans-7"
  },

  /* [17] sdm-7 @ 128×44100 */
  {
    {
      7.83148010097334e-01,
      3.01437231238902e-01,
      7.31891646224574e-02,
      1.20314098366875e-02,
      1.32077937193861e-03,
      9.11181979169687e-05,
      2.59895240306562e-06,
    },
    {
      0, 9.92163123766340e-05,
      0, 3.31199917300393e-04,
      0, 5.42540771343282e-04,
    },
    7, 128 * 44100, "sdm-7"
  },

  /* [18] clans-8 @ 128×44100 */
  {
    {
      1.04472698053970e+00,
      4.62088167600438e-01,
      1.13484722685479e-01,
      1.68939738398161e-02,
      1.55891676875336e-03,
      8.23864822188133e-05,
      2.39690238375972e-06,
      -1.75063180618551e-09,
    },
    {
      2.02698799324546e-05, 0,
      1.66362887238597e-04, 0,
      3.82276797905696e-04, 0,
      5.55397776875272e-04, 0,
    },
    8, 128 * 44100, "clans-8"
  },

  /* [19] sdm-8 @ 128×44100 */
  {
    {
      7.42763211426562e-01,
      2.71983157679393e-01,
      6.36389361390464e-02,
      1.03289230528372e-02,
      1.19045645863092e-03,
      9.25357160397986e-05,
      4.64982367004083e-06,
      8.14280266547840e-08,
    },
    {
      2.02698799324546e-05, 0,
      1.66362887238597e-04, 0,
      3.82276797905696e-04, 0,
      5.55397776875272e-04, 0,
    },
    8, 128 * 44100, "sdm-8"
  },

  /* ═══════════════════════════════════════════════════════════════
   * 64 × 44100  (DSD64 = 2822400 Hz)
   * ═══════════════════════════════════════════════════════════════ */

  /* [20] clans-4 @ 64×44100 */
  {
    {
      1.27879853057675e+00,
      6.11303913722028e-01,
      1.28497083869344e-01,
      9.36669621421730e-03,
    },
    {
      2.78489536971958e-04, 0,
      1.78576750808173e-03, 0,
    },
    4, 64 * 44100, "clans-4"
  },

  /* [21] sdm-4 @ 64×44100 */
  {
    {
      8.70691905361989e-01,
      3.56902669565715e-01,
      7.90540396115068e-02,
      7.49922172520510e-03,
    },
    {
      2.78489536971958e-04, 0,
      1.78576750808173e-03, 0,
    },
    4, 64 * 44100, "sdm-4"
  },

  /* [22] clans-5 @ 64×44100 */
  {
    {
      1.09979653514762e+00,
      4.81149952106030e-01,
      1.03481231987752e-01,
      1.07520561970131e-02,
      3.08801118488355e-04,
    },
    {
      0, 6.98490600683106e-04,
      0, 1.97734357803445e-03,
    },
    5, 64 * 44100, "clans-5"
  },

  /* [23] sdm-5 @ 64×44100 */
  {
    {
      8.09008352716413e-01,
      3.14889441429587e-01,
      7.21235639855639e-02,
      9.61769014140330e-03,
      5.16726747047100e-04,
    },
    {
      0, 6.98490600683106e-04,
      0, 1.97734357803445e-03,
    },
    5, 64 * 44100, "sdm-5"
  },

  /* [24] clans-6 @ 64×44100 */
  {
    {
      1.07903996429881e+00,
      4.81889508657128e-01,
      1.12960470418260e-01,
      1.41786764681378e-02,
      8.90696638761455e-04,
      3.12209321540191e-06,
    },
    {
      1.37194204516672e-04, 0,
      1.05309113432481e-03, 0,
      2.09365847953595e-03, 0,
    },
    6, 64 * 44100, "clans-6"
  },

  /* [25] sdm-6 @ 64×44100 */
  {
    {
      8.10379824203071e-01,
      3.18536209193388e-01,
      7.66325098232035e-02,
      1.16280270347611e-02,
      1.07113013239551e-03,
      3.23564051386283e-05,
    },
    {
      1.37194204516672e-04, 0,
      1.05309113432481e-03, 0,
      2.09365847953595e-03, 0,
    },
    6, 64 * 44100, "sdm-6"
  },

  /* [26] clans-7 @ 64×44100 */
  {
    {
      1.30828743581024e+00,
      6.14252690035661e-01,
      1.30284958810903e-01,
      1.31280998331490e-02,
      4.80497172614556e-04,
      1.28747977598542e-07,
      -1.01500259908072e-06,
    },
    {
      0, 3.96825873999969e-04,
      0, 1.32436089566069e-03,
      0, 2.16898568341885e-03,
    },
    7, 64 * 44100, "clans-7"
  },

  /* [27] sdm-7 @ 64×44100 */
  {
    {
      7.84599817960974e-01,
      2.99634346028983e-01,
      7.13049276218066e-02,
      1.13334107086916e-02,
      1.14497642818158e-03,
      7.33018502803093e-05,
      6.80633400002018e-07,
    },
    {
      0, 3.96825873999969e-04,
      0, 1.32436089566069e-03,
      0, 2.16898568341885e-03,
    },
    7, 64 * 44100, "sdm-7"
  },

  /* [28] clans-8 @ 64×44100 */
  {
    {
      1.18730059129261e+00,
      5.66733317291325e-01,
      1.40117339676942e-01,
      1.87599862200771e-02,
      1.27685506908071e-03,
      8.76397405988154e-06,
      -1.90294986721073e-06,
      -7.39020160622772e-08,
    },
    {
      8.10778762576884e-05, 0,
      6.65340842513387e-04, 0,
      1.52852264942192e-03, 0,
      2.22035724073886e-03, 0,
    },
    8, 64 * 44100, "clans-8"
  },

  /* [29] sdm-8 @ 64×44100 */
  {
    {
      7.44453769826547e-01,
      2.69850507860307e-01,
      6.16093616071757e-02,
      9.52771711245796e-03,
      1.02903114196526e-03,
      6.63758229311911e-05,
      2.91124056073927e-06,
      -4.29323230577427e-08,
    },
    {
      8.10778762576884e-05, 0,
      6.65340842513387e-04, 0,
      1.52852264942192e-03, 0,
      2.22035724073886e-03, 0,
    },
    8, 64 * 44100, "sdm-8"
  },
};

#define FILTER_COUNT (int)(sizeof(sdm_filters) / sizeof(sdm_filters[0]))

/* Map ntf_filter_id_t → (name, order) */
static const struct { const char *name; int order; } id_map[] = {
    [NTF_CLANS_4] = { "clans-4", 4 },
    [NTF_SDM_4]   = { "sdm-4",   4 },
    [NTF_CLANS_5] = { "clans-5", 5 },
    [NTF_SDM_5]   = { "sdm-5",   5 },
    [NTF_CLANS_6] = { "clans-6", 6 },
    [NTF_SDM_6]   = { "sdm-6",   6 },
    [NTF_CLANS_7] = { "clans-7", 7 },
    [NTF_SDM_7]   = { "sdm-7",   7 },
    [NTF_CLANS_8] = { "clans-8", 8 },
    [NTF_SDM_8]   = { "sdm-8",   8 },
};

/*
 * Lookup by ID and DSD rate.
 * Matches the reference sdm_find_filter logic: name match + freq <= dsd_rate.
 */
const ntf_filter_t *ntf_get_filter(ntf_filter_id_t id, unsigned dsd_rate) {
    if (id < 0 || id >= NTF_COUNT)
        return NULL;

    const char *name = id_map[id].name;
    if (!name)
        return NULL;

    for (int i = 0; i < FILTER_COUNT; i++) {
        if (strcmp(sdm_filters[i].name, name) == 0 &&
            sdm_filters[i].freq <= dsd_rate) {
            return &sdm_filters[i];
        }
    }

    return NULL;
}

/*
 * Auto-select: CLANS-N where N matches the DSD rate tier.
 *   DSD64  → CLANS-5 @ 64×44100
 *   DSD128 → CLANS-6 @ 128×44100
 *   DSD256 → CLANS-7 @ 256×44100
 *   DSD512 → CLANS-8 @ 256×44100 (highest available freq group)
 */
const ntf_filter_t *ntf_auto_select(unsigned dsd_rate) {
    /* DSD64/DSD128: keep conservative NTF orders — higher orders cause the
     * trellis to struggle at low DSD rates (insufficient noise shaping bandwidth).
     * DSD256/DSD512: +1 order gives large SINAD improvement. */
    ntf_filter_id_t id;

    if (dsd_rate >= DSD_RATE_512)
        id = NTF_CLANS_8;
    else if (dsd_rate >= DSD_RATE_256)
        id = NTF_CLANS_8;
    else if (dsd_rate >= DSD_RATE_128)
        id = NTF_CLANS_6;
    else
        id = NTF_CLANS_5;

    return ntf_get_filter(id, dsd_rate);
}

const ntf_filter_t *ntf_auto_select_precorr(unsigned dsd_rate) {
    /* PreCorr's greedy quantizer benefits from +1 NTF order vs Trellis.
     * DSD64→clans-6, DSD128→clans-7, DSD256→clans-8, DSD512→clans-7.
     * DSD512 clans-8 is unstable for PreCorr's greedy quantizer. */
    ntf_filter_id_t id;

    if (dsd_rate >= DSD_RATE_512)
        id = NTF_CLANS_7;
    else if (dsd_rate >= DSD_RATE_256)
        id = NTF_CLANS_7;  /* clans-8 unstable with boxcar input at DSD256 */
    else if (dsd_rate >= DSD_RATE_128)
        id = NTF_CLANS_7;
    else
        id = NTF_CLANS_6;

    return ntf_get_filter(id, dsd_rate);
}

int ntf_get_filter_count(void) {
    return FILTER_COUNT;
}

const ntf_filter_t *ntf_get_by_index(int index) {
    if (index < 0 || index >= FILTER_COUNT)
        return NULL;
    return &sdm_filters[index];
}
